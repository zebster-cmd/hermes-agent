...
def _make_update_side_effect(
    ...
    fetch_fails=False,
    fetch_stderr="",
    fetch_ref_fails=False,
    branch_force_fails=False,
):
    """Build a subprocess.run side_effect for cmd_update tests.

    The updated cmd_update behaviour (PR #3216):
    * On the target branch: pull --ff-only, fallback to pull --rebase.
    * On a different branch: fetch origin {branch}:{branch}, fallback to
      branch -f {branch} origin/{branch} (no checkout).
    """
    recorded = []
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "fetch" in joined and "origin" in joined:
            ...
            # Distinguish plain "fetch origin" from "fetch origin main:main"
            if ":" in joined:
                if fetch_ref_fails:
                    return SimpleNamespace(stdout="", stderr="! [rejected]\n", returncode=1)
                return SimpleNamespace(stdout="", stderr="", returncode=0)
            return SimpleNamespace(stdout="", stderr="", returncode=0)
        if "rev-parse" in joined and "--abbrev-ref" in joined:
            return SimpleNamespace(stdout=f"{current_branch}\n", stderr="", returncode=0)
        if "rev-parse" in joined and "--verify" in joined:
            # origin/{branch} exists check — succeed only for main
            if "origin/main" in joined:
                return SimpleNamespace(stdout="abc123\n", stderr="", returncode=0)
            return SimpleNamespace(stdout="", stderr="fatal: not found\n", returncode=128)
        ...
        if "--rebase" in joined:
            if rebase_fails:
                raise CalledProcessError(returncode=1, cmd=cmd)
            return SimpleNamespace(stdout="Updating abc..def\n", stderr="", returncode=0)
        if "branch" in joined and "-f" in joined:
            if branch_force_fails:
                raise CalledProcessError(returncode=1, cmd=cmd)
            return SimpleNamespace(stdout="", stderr="", returncode=0)
...
def test_cmd_update_falls_back_to_rebase_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    """When --ff-only fails (diverged history), update falls back to pull --rebase."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
...
# ---------------------------------------------------------------------------
# Non-main branch → update ref without checkout (PR #3216)
# ---------------------------------------------------------------------------

def test_cmd_update_stays_on_feature_branch(monkeypatch, tmp_path, capsys):
    """When on a feature branch, update fetches main ref without checkout."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    # Should NOT have checked out main
    checkout_calls = [c for c in recorded if "checkout" in c and "main" in c]
    assert len(checkout_calls) == 0

    # Should have used fetch origin main:main to update the ref
    fetch_ref_calls = [c for c in recorded if "fetch" in c and "main:main" in " ".join(str(x) for x in c)]
    assert len(fetch_ref_calls) == 1

    out = capsys.readouterr().out
    assert "staying on fix/something" in out.lower() or "Updated local main" in out


def test_cmd_update_uses_branch_force_on_diverged_ref(monkeypatch, tmp_path, capsys):
    """When fetch origin main:main fails (diverged), falls back to branch -f."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)

    side_effect, recorded = _make_update_side_effect(
        current_branch="fix/something", fetch_ref_fails=True,
    )
    monkeypatch.setattr(hermes_main.subprocess, "run", side_effect)

    hermes_main.cmd_update(SimpleNamespace())

    branch_force_calls = [c for c in recorded if "branch" in c and "-f" in c]
    assert len(branch_force_calls) == 1
    assert "main" in branch_force_calls[0]
    assert "origin/main" in branch_force_calls[0]


def test_cmd_update_returns_early_when_already_up_to_date_on_feature_branch(monkeypatch, tmp_path, capsys):
    """When on a feature branch with no updates, exits early without stash."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    # No checkout should have happened at all
...
def test_cmd_update_skips_stash_restore_when_rebase_fails(monkeypatch, tmp_path, capsys):
    """When both --ff-only and --rebase fail, stash restore is skipped."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    with pytest.raises((SystemExit, CalledProcessError)):
        hermes_main.cmd_update(SimpleNamespace())
